...
parameters{
  ...
  vector<lower=0>[D] std_par;//transformed intercepts
}
transformed parameters{
  //vector[S] r_1_1 = ()
  vector<lower=0>[S] theta_temp;
  //vector[D] R0; // random intercept
  
  //R0 = mu0 + sigma0 * std_par;
  
  for (s0 in 1:S){
    theta_temp[s0] = exp(mu0 + sigma0 * std_par[id[s0]] + X_centered[s0,]*R1_K);
  }
}
model{
  int position = 1;
...
generated quantities{
  ...
  vector[D] R0_true = mu0 + sigma0 * std_par - dot_product(X_means, R1_K);
